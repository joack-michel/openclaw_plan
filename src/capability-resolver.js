...
export function resolveCapability(event, ctx = {}, config = {}) {
  ...
  if (isNativeReadOnlyTool(toolName, params)) {
    return capability("READ_AUTOMATION_STATE", "L0", toolName, source, params);
  }
  ...
  if (toolName === "exec") {
    return execCapability(toolName, source, params, { ...ctx, runtimePolicyMode: config.runtimePolicyMode || ctx.runtimePolicyMode });
  }
  ...
  // MCP readOnlyHint is Gateway-supplied tool metadata, not a model-provided
  // tool-name guess.  It is the generic path for side-effect-free MCP calls.
  if (isExplicitReadOnlyMcp(toolName, ctx)) return capability("QUERY_EXTERNAL", "L0", toolName, source, params);
  ...
  if (/^memory_(?:get|search|list)$/i.test(toolName)) return capability("READ_AUTOMATION_STATE", "L0", toolName, source, params);
...
function execCapability(toolName, source, params, ctx) {
  const command = String(params.command || "").trim();
  const effect = resolveExecEffect(params, ctx);
  const argv = effect.parsed?.commands?.length === 1 ? effect.parsed.commands[0].argv : [];
  return capability(effect.capability, effect.riskLevel, toolName, source, params, {
    ...
    executable: argv[0] || "",
    argvShape: argv.length > 1 ? [argv[0], ...argv.slice(1).map(() => "<arg>")] : argv,
    commandHash: sha256Hex(stableStringify({ command })),
    scriptIdentity: effect.scriptIdentity,
    execEffectReason: effect.reason,
    denialCode: effect.denialCode || "",
    trustedLocal: effect.trustedLocal === true,
    registeredSkill: effect.registeredSkill === true,
    skillId: effect.skillId || "",
    skillExecution: effect.skillExecution || {},
    skillSnapshotHash: effect.skillSnapshotHash || "",
    parsed: effect.parsed || null
  });
}
...
function operationTypeFor(name) {
  ...
  if (name === "NUMBER_QUERY") return "readonly";
  if (name === "NUMBER_AUTH_START") return "number-auth-start";
  if (name === "NUMBER_AUTH_SUBMIT") return "number-auth-submit";
...
function isExplicitReadOnlyMcp(toolName, ctx) {
  if (!String(toolName || "").includes("__")) return false;
  const annotations = ctx?.toolDefinition?.annotations || ctx?.tool?.annotations || ctx?.mcpTool?.annotations || {};
...
function isNativeReadOnlyTool(toolName, params) {
  if (["session_status", "sessions_list", "sessions_search", "web_search"].includes(toolName)) return true;
  if (toolName === "nodes") return /^(status|list|get|describe)$/i.test(String(params.action || params.method || "status"));
  if (toolName === "subagents") return /^(list|status)$/i.test(String(params.action || "list"));
  return false;
}
