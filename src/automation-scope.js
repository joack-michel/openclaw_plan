...
import { BENEFITS_PARENT_CRON_JOB_ID, DEFAULT_AGENT_IDS, MEITUAN_CRON_JOB_ID, OWNER_ID, OWNER_TELEGRAM_ID, REM_CRON_JOB_ID, openclawPath, workspacePath } from "./template-config.js";
...
const REM_READ_DISCOVERY_COMMANDS = [
  `find ${workspacePath("memory", "topics")} -type f`,
  `find ${workspacePath("memory", ".learnings")} -type f`,
  `find ${workspacePath("topics")} -type f`,
  `find ${workspacePath(".learnings")} -type f`
];
...
export function buildAutomationGrantFromCron(job, now = Date.now()) {
  ...
  return {
    ...
    maxRunsPerPeriod: maxRunsPerDay(job),
    periodKind: "day",
...
export function inferScope(job) {
  ...
  const isMeituanCouponJob = job.id === MEITUAN_CRON_JOB_ID && job.agentId === DEFAULT_AGENT_IDS.meituan;
  const isMcdCouponJob = /mcd-mcp__|麦当劳/i.test(text);
  const isBenefitsParent = job.id === BENEFITS_PARENT_CRON_JOB_ID && job.agentId === DEFAULT_AGENT_IDS.benefits;
  if (isBenefitsParent) {
  ...
  const isRemMemoryScan = job.id === REM_CRON_JOB_ID && job.agentId === DEFAULT_AGENT_IDS.rem;
  if (isRemMemoryScan) {
...
function maxRunsPerDay(job) {
  if (job.id === REM_CRON_JOB_ID && job.agentId === DEFAULT_AGENT_IDS.rem) return 12;
