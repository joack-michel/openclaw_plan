...
for arg in "$@"; do
  case "$arg" in
    ...
    *) printf 'usage: %s [--release] [--skip-tests] [--history]\n' "$0" >&2; exit 2 ;;
  esac
done
...
if $scan_history; then
  printf 'Verification passed: current release tree and recoverable Git objects are clear.\n'
else
  printf 'Verification passed: current release tree is clear.\n'
fi
